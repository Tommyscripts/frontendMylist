  ...
  <v-container>
    <v-row>
      <v-col
        ...
        md="4"
        sm="6"
        v-for="(embutido, idx) in embutidos"
        :key="idx"
      >
        <v-card class="color">
              ...
              <v-menu open-on-hover>
                ...
                <v-list nav>
                  <v-list-item
                    ...
                    :key="index"
                    @click="agregarProducto(list.name, embutido._id)"
                  >
                    <v-list-item-title>
                      {{ list.name }}
                    </v-list-item-title>
...
export default {
  ...
  async created() {
    ...
    if (Array.isArray(result)) {
      ...
      this.productos.forEach((el) => { if (el.categorias === "Embutidos") this.embutidos.push(el) })
    } else {
  ...
  methods: {
    ...
    async agregarProducto(name, id) {
      // Delegamos la lógica al helper central para mantener consistencia en todas las vistas
