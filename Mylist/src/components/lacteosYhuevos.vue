<template>
  <v-container>
    <v-row>
      <v-col cols="12" md="4" sm="6" v-for="(lacteo, idx) in lacteos" :key="idx">
        <v-card class="color">
          <v-card-title class="d-flex">
            <v-avatar>
              <v-img :src="lacteo.img"> </v-img>
            </v-avatar>
            {{ lacteo.name }}
            <v-card-item>
              <v-menu open-on-hover>
                ...
                <v-list nav>
                  <v-list-item
                    ...
                    :key="index"
                    @click="agregarProducto(list.name, lacteo._id)"
                  >
                    <v-list-item-title>
                      {{ list.name }}
                    </v-list-item-title>
...
export default {
  ...
  async created() {
    ...
    if (Array.isArray(result)) {
      ...
      this.productos.forEach((el) => { if (el.categorias === "Lacteos y huevos") this.lacteos.push(el) })
    } else {
