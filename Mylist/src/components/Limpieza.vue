<template>
  <v-container>
    <v-row>
      <v-col cols="12" md="4" sm="6" v-for="(limpieza, idx) in limpiezas" :key="idx">
        <v-card class="color">
              ...
              <v-menu open-on-hover>
                ...
                <v-list nav>
                  <v-list-item
                    ...
                    :key="index"
                    @click="agregarProducto(list.name, limpieza._id)"
                  >
                    <v-list-item-title>
                      {{ list.name }}
                    </v-list-item-title>
  ...
  async created() {
    ...
    if (Array.isArray(result)) {
      ...
      this.productos.forEach((el) => {
        if (el.categorias === "Limpieza") this.limpiezas.push(el);
      });
    } else {
