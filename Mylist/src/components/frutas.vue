<template>
  <v-container>
    <v-row>
      <v-col cols="12" md="4" sm="6" v-for="(fruta, idx) in frutas" :key="idx">
        <v-card class="color">
          <v-card-title class="d-flex">
            <v-avatar>
              <v-img :src="fruta.img"> </v-img>
            </v-avatar>
            {{ fruta.name }}
            <v-card-item>
              <v-menu open-on-hover>
                ...
                <v-list nav>
                  <v-list-item
                    ...
                    :key="index"
                    @click="agregarProducto(list.name, fruta._id)"
                  >
                    <v-list-item-title>
                      {{ list.name }}
                    </v-list-item-title>
  ...
  async created() {
    ...
    if (Array.isArray(result)) {
      ...
      this.productos.forEach((el) => {
        if (el.categorias === "Frutas") this.frutas.push(el);
      });
    } else {
